...
namespace BudgetPilot_API.Services
{
    ...
    public class CardsService
    {
        ...
        public async Task<CardsOBJ> CreateCard(CardsDTO dto, Guid userId)
        {
            ...
            try
            {
                encryptedCardNumber = _dataProtection.Encrypt(dto.CardNumber);
                encryptedCvc = _dataProtection.Encrypt(dto.Cvc);
            }
            catch (CryptographicException ex)
            {
                throw new InvalidOperationException("Failed to encrypt card data. Please verify the encryption key configuration.", ex);
            }
        ...
        /// <summary>
        /// Reactivates an inactive card. Returns whether the card was found
        /// and whether it was already active.
        /// </summary>
        public async Task<(bool Found, bool AlreadyActive)> ReactivateCard(Guid id)
        {
            var card = await _context.Cards
        ...
        /// <summary>
        /// Retrieves all cards for a specific user, optionally filtered by active status.
        /// Decrypts card numbers and CVCs. Used by admin endpoints.
        /// </summary>
